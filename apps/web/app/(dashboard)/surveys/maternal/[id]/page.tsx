...
export default async function MaternalSurveyDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const survey = await prisma.maternalHealthSurvey.findUnique({ where: { id } });
  if (!survey) notFound();
        ...
        <dl className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {(
            [
              ...
              ["Health Facility", survey.healthFacility ?? "—"],
