...
const CATEGORY_LABELS: Record<string, string> = {
  scholarship_beneficiary: "Scholarship Beneficiary",
  bam_participant: "BAM Participant",
  gam_participant: "GAM Participant",
...
export default async function EducationSurveysPage({
  searchParams,
}: {
  ...
  const where: { communityOrSchool?: string; respondentCategory?: string } = {};
  if (params.community) where.communityOrSchool = params.community;
  if (params.category) where.respondentCategory = params.category;

  const [surveys, total, categoryCounts] = await Promise.all([
    prisma.educationSurvey.findMany({
      where,
    ...
    prisma.educationSurvey.groupBy({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-ft-grey-darker">Education Surveys</h1>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-ft-grey-darker text-white">
            <tr>
              ...
              <th className="px-4 py-2">Community / School</th>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 text-sm">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              key={p}
              href={{ pathname: "/surveys/education", query: { ...params, page: p } }}
