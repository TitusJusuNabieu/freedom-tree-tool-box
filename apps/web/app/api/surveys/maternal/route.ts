...
export async function GET(req: NextRequest) {
  ...
  const where: Prisma.MaternalHealthSurveyWhereInput = {};
  ...
  const [surveys, total] = await Promise.all([
    prisma.maternalHealthSurvey.findMany({ where, orderBy: { surveyDate: "desc" }, skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE }),
